...
const StoreDisputeDetailPage = () => {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    ...
    () => getStoreDisputeDetail(currentStore.id, disputeId),
    { 
        ...
        <div className="bg-gray-50 min-h-screen py-6">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-white rounded-lg shadow p-6 text-center">
              <p className="text-red-600">Vui lòng chọn cửa hàng</p>
            </div>
          </div>
        </div>
      </StoreLayout>
  ...
  return (
    <StoreLayout>
      <SEO
        ...
        description="Xem chi tiết khiếu nại"
        keywords="khiếu nại, chi tiết khiếu nại"
        url={`https://pbl-6-eight.vercel.app/store-dashboard/returns/disputes/${disputeId}`}
      />
      <div className="bg-gray-50 min-h-screen py-6">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Header */}
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">Xem và quản lý khiếu nại</p>
            </div>
            <Link
              to="/store-dashboard/returns"
            ...
            <div className="space-y-6">
              ...
              <div ref={infoSectionRef} className="bg-white rounded-lg shadow p-6">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  {/* Thông báo và button khi 2 bên tự thống nhất */}
                  {dispute.status === 'OPEN' && dispute.disputeType === 'RETURN_REJECTION' && (
                    <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg space-y-3">
                      <p className="text-sm text-yellow-800">
                        💡 <strong>Lưu ý:</strong> Nếu bạn và người mua đã thống nhất:
                      </p>
                      <div className="flex flex-wrap gap-2">
                        ...
                        >
                          Chấp nhận lại yêu cầu trả hàng (Người mua thắng)
                        </Link>
                        <button
                          onClick={async () => {
                            ...
                            if (confirmed) {
                              ...
                              if (result.success) {
                                showSuccess('Đã gửi tin nhắn xác nhận. Vui lòng chờ Admin giải quyết để đóng khiếu nại.');
                                mutate();
                              } else {
                                showError(result.error || 'Không thể gửi tin nhắn');
                              }
                        ...
                        >
                          Gửi tin nhắn xác nhận (Người bán thắng)
                        </button>
                      </div>
                      <p className="text-xs text-yellow-700 mt-2">
                        <strong>Lưu ý:</strong> Hiện tại BE chưa có API để tự đóng khiếu nại. Sau khi gửi tin nhắn xác nhận, Admin sẽ vào giải quyết để đóng khiếu nại chính thức.
                      </p>
                    </div>
                  )}
                  
                  {dispute.status === 'OPEN' && dispute.disputeType === 'RETURN_QUALITY' && (
                    <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                      <p className="text-sm text-yellow-800">
                        💡 <strong>Lưu ý:</strong> Nếu bạn và người mua đã thống nhất, bạn có thể xác nhận hàng OK để tự giải quyết khiếu nại.
                      </p>
                    </div>
                  )}
                </div>
                ...
                {dispute.messages && dispute.messages.length > 0 ? (
                  <div className="space-y-4 mb-6 max-h-96 overflow-y-auto" id="messages-container">
                    {dispute.messages.map((message, index) => {
                      ...
                      const senderName = message.senderName || (isStore ? 'Cửa hàng' : isAdmin ? 'Admin' : 'Người mua');
                      ...
                      return (
                        <div key={index} className={containerClasses}>
                          ...
                          {/* Hiển thị file đính kèm theo từng tin nhắn */}
                          ...
                          <div key={idx} className="relative group">
                            {isImage ? (
                              <a
                                ...
                                onClick={(e) => {
                                  e.preventDefault();
                                  setPreviewAttachment({ url, type: 'image' });
                                }}
                              ...
                              <a
                                ...
                                onClick={(e) => {
                                  e.preventDefault();
                                  setPreviewAttachment({ url, type: 'video' });
                                }}
                              ...
                              <a
                                ...
                                onClick={(e) => {
                                  e.preventDefault();
                                  setPreviewAttachment({ url, type: 'file' });
                                }}
                                className="block aspect-square rounded-lg overflow-hidden border border-gray-200 hover:border-blue-400 transition bg-gray-100 flex items-center justify-center cursor-zoom-in"
                              >
